...
class phpBrainz{	
	...
	public function findRelease(phpBrainz_ReleaseFilter $releaseFilterObj){
	    if(sizeof($releaseFilterObj->createParameters())<1){
	        die(
	        print("The release filter object needs 
	...
	public function findTrack(phpBrainz_TrackFilter $trackFilterObj){
	    if(sizeof($trackFilterObj->createParameters())<1){
	       die(
	       print(
	       "At least one argument must be supplied to performa a query."));
	    }    
	    ...
	    foreach($xml->{'track-list'}->track as $track){
	        $attrNS = 
	           $track->attributes("http://musicbrainz.org/ns/ext-1.0#");
	        $trackRels = array();
	...
	/**
	 * Returns the information specified in the 
	 * $trackIncludes array 
	 *
	 * @param string $mbid
	...
	public function getTrack($mbid, $trackIncludes=null){
	   ...
	   foreach($trackIncludes as $include){
	       $url .= $include."+";
	   }
	   $url = rtrim($url,"+");
	...
	public function getRelease($mbid, $albumIncludes){
	   ...
	   foreach($albumIncludes as $include){
	       $url .= $include."+";
	   }
	   $url = rtrim($url,"+");
	   $xml = simplexml_load_file($url);
	 ...
	 * @return boolean
	 */
	private function isValidMBID($str){
